...
export const MainContent = (props: { section: string }) => {
  ...
  return <JumboBox>{sectionContent}</JumboBox>;
};
...
export const Home = () => {
  return (
    <Container>
      <hr />
      <h2>{homeContent.websiteTitle}</h2>
      <hr />
      <h4>{homeContent.title}</h4>
      <p>{homeContent.description}</p>
      <div>
        <a href="https://info.flagcounter.com/OTDv">
          <img
            src="https://s05.flagcounter.com/countxl/OTDv/bg_FFFFFF/txt_000000/border_787878/columns_3/maxflags_12/viewers_0/labels_0/pageviews_0/flags_0/percent_0/"
            alt="Flag Counter"
          />
        </a>
      </div>
...
export const Experience = () => {
  ...
  return (
    <div>
      <h4>General working experience:</h4>
      <RetroTableWrapper>
        <table>
          <tr>
            <th>{'Company'}</th>
          ...
          <tr>
            ...
            {redSharpJob.map((dataItem, indexRow) => (
              <td key={indexRow}>{dataItem}</td>
            ))}
          </tr>
          <tr>
            <td>{Object.keys(experienceContent)[1]}</td>
            {svJob.map((dataItem, indexRow) => (
              <td key={indexRow}>{dataItem}</td>
            ))}
          </tr>
        </table>
...
export const About = () => {
  const structStudies: any = studies;
